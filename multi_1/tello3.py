...
class Tello3:
	...
	def __init__(self):
		...
		# Socket for commands, video and state. IPv4, with UDP
		self.socket_cmd = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		self.socket_cmd.setsockopt(socket.SOL_SOCKET, 25, 'wlx503eaadfae67'.encode())
		self.socket_video = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		self.socket_video.setsockopt(socket.SOL_SOCKET, 25, 'wlx503eaadfae67'.encode())
		self.socket_state = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		self.socket_state.setsockopt(socket.SOL_SOCKET, 25, 'wlx503eaadfae67'.encode())

		# Bind to enable communication and listening
		# (Video bind only required for H264Decoder in startVideoCapture method)
		self.socket_cmd.bind(Tello3.LOCAL_ADDRESS)	# comms
		self.socket_state.bind(Tello3.STATE_ADDRESS)	# listening for state
	...
	def startVideoCapture(self, method="H264Decoder"):
		...
		if method == "H264Decoder":
			...
			self.socket_video.bind(Tello3.VIDEO_ADDRESS)
			...
			self.cap = cv.VideoCapture(Tello3.camera_address)
	...
	def sendCommand(self, command):
		...
		self.socket_cmd.sendto(command.encode('utf8'), Tello3.TELLO_ADDRESS)
		time.sleep(Tello3.command_timeout)
