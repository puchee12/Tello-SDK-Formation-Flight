...
def runWaypoint(true_state, streamingClient, r_wd, dt, tello3):
	'''
	...
	try:
		controllerWaypoint(error, runWaypoint.prev_error, dt, tello3)
		# PID(error, runWaypoint.prev_error, dt, tello)

	except AttributeError:
		controllerWaypoint(error, error, dt, tello3)		# first run
		# PID(error, error, dt, tello3)		# first run
	runWaypoint.prev_error = error
	return error


def controllerWaypoint(error, prev_error, dt, tello3):
	...
	tello3.rc(lr=int(pid_input[0]), fb=int(pid_input[1]), ud=-int(pid_input[2]), yaw=int(pid_input[3]))
	# tello3.rc(lr=int(pid_input[0]), fb=int(pid_input[1]), ud=-int(pid_input[2]), yaw=int(0))



def PID(error, prev_error, dt, tello3):
