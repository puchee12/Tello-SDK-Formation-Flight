...
class NatNetClient:
    def __init__( self, body_id_drone1, body_id_drone2):
        ...
        self.body_id_drone1 = body_id_drone1
        self.body_id_drone2 = body_id_drone2
    ...
    def __unpackRigidBody( self, data):
        ...
        if self.rigidBodyListener is not None and id == self.body_id_drone1:
            # self.rigidBodyListener( id, pos, rot )
            self.rigidBodyListener[0] = np.array([id, np.array(pos), np.array(rot)])

        if self.rigidBodyListener is not None and id == self.body_id_drone2:
